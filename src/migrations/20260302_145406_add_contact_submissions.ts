...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`reviews\` (
  	...
  	\`name\` text DEFAULT 'Khách hàng' NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`destinations_highlights\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`text\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`destinations_highlights_order_idx\` ON \`destinations_highlights\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`destinations_highlights_parent_id_idx\` ON \`destinations_highlights\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`destinations\` (
  	...
  	\`tag\` text,
  	\`emoji\` text,
  	\`gradient\` text DEFAULT 'from-emerald-500 to-green-700',
  	\`image_id\` integer,
  ...
  await db.run(sql`CREATE INDEX \`destinations_image_idx\` ON \`destinations\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`rooms_amenities\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`rooms_amenities_order_idx\` ON \`rooms_amenities\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`rooms_amenities_parent_id_idx\` ON \`rooms_amenities\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`rooms\` (
  	...
  	\`price_unit\` text DEFAULT 'đêm/phòng',
  	\`capacity\` text,
  	\`highlight\` text,
  	\`highlight_color\` text DEFAULT 'bg-primary',
  	\`emoji\` text,
  	\`image_id\` integer,
  	\`booking_link\` text,
  	\`order\` numeric DEFAULT 0,
  ...
  await db.run(sql`CREATE INDEX \`rooms_image_idx\` ON \`rooms\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`variants\` (
  	...
  	\`price_in_v_n_d_enabled\` integer,
  	\`price_in_v_n_d\` numeric,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`deleted_at\` text,
  	\`_status\` text DEFAULT 'draft',
  ...
  await db.run(sql`CREATE TABLE \`_variants_v\` (
  	...
  	\`version_price_in_v_n_d_enabled\` integer,
  	\`version_price_in_v_n_d\` numeric,
  	\`version_updated_at\` text,
  	\`version_created_at\` text,
  ...
  await db.run(sql`CREATE TABLE \`_products_v\` (
  	...
  	\`version_unit\` text DEFAULT 'kg',
  	\`version_usage_info\` text,
  	\`version_detail_info\` text,
  	\`version_meta_title\` text,
  	\`version_meta_image_id\` integer,
  	\`version_meta_description\` text,
  	\`version_generate_slug\` integer DEFAULT true,
  ...
  await db.run(sql`CREATE TABLE \`orders\` (
  	...
  	\`shipping_address_province_id\` numeric,
  	\`shipping_address_province_name\` text,
  	\`shipping_address_district_id\` numeric,
  	\`shipping_address_district_name\` text,
  	\`shipping_address_ward_code\` text,
  	\`shipping_address_ward_name\` text,
  	\`customer_id\` integer,
  	\`customer_email\` text,
  ...
  await db.run(sql`CREATE TABLE \`transactions\` (
  	...
  	\`payment_method\` text,
  	\`cod_cod_token\` text,
  	\`vnpay_vnpay_txn_ref\` text,
  	\`vnpay_vnpay_transaction_no\` text,
  	\`vnpay_vnpay_bank_code\` text,
  	\`vnpay_vnpay_response_code\` text,
  	\`momo_momo_order_id\` text,
  	\`momo_momo_trans_id\` text,
  	\`momo_momo_pay_type\` text,
  	\`momo_momo_result_code\` text,
  	...
  	\`billing_address_province_id\` numeric,
  	\`billing_address_province_name\` text,
  	\`billing_address_district_id\` numeric,
  	\`billing_address_district_name\` text,
  	\`billing_address_ward_code\` text,
  	\`billing_address_ward_name\` text,
  	\`shipping_address_first_name\` text,
  	\`shipping_address_last_name\` text,
  	\`shipping_address_phone\` text,
  	\`shipping_address_address_line1\` text,
  ...
  await db.run(sql`CREATE TABLE \`home_page_why_choose_us\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`icon\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`home_page_why_choose_us_order_idx\` ON \`home_page_why_choose_us\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`home_page_why_choose_us_parent_id_idx\` ON \`home_page_why_choose_us\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`home_page_testimonials\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`customer_name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`home_page_testimonials_order_idx\` ON \`home_page_testimonials\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`home_page_testimonials_parent_id_idx\` ON \`home_page_testimonials\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`home_page\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`hero_section_headline\` text NOT NULL,
  	\`hero_section_subheadline\` text,
  	\`hero_section_background_image_id\` integer,
  	\`hero_section_cta_text\` text,
  	\`hero_section_cta_link\` text,
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`hero_section_background_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`home_page_hero_section_hero_section_background_image_idx\` ON \`home_page\` (\`hero_section_background_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`site_name\` text DEFAULT 'Hồng Thái Na Hang' NOT NULL,
  	\`logo_id\` integer,
  	\`hotline\` text DEFAULT '0988 456 789',
  	\`zalo\` text DEFAULT '0988 456 789',
  	\`email\` text DEFAULT 'info@hongthai-nahang.vn',
  	\`address\` text DEFAULT 'Xã Hồng Thái, Na Hang, Tuyên Quang',
  	\`facebook_page\` text,
  	\`messenger_link\` text DEFAULT 'https://m.me/hongthai.nahang',
  	\`updated_at\` text,
  	\`created_at\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`destinations_highlights\`;`)
  await db.run(sql`DROP TABLE \`destinations\`;`)
  await db.run(sql`DROP TABLE \`rooms_amenities\`;`)
  ...
  await db.run(sql`DROP TABLE \`home_page_why_choose_us\`;`)
  await db.run(sql`DROP TABLE \`home_page_testimonials\`;`)
